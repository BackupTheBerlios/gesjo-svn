...
class GJLib:
	...
	def __init__(self,bd):
		...
		self.generadors["cl"] = "GEN_CLI_ID"
		self.generadors["ma"] = "GEN_MAR_ID"
		self.generadors["ta"] = "GEN_TAL_ID"
		self.generadors["me"] = "GEN_MEC_ID"
		self.generadors["ar"] = "GEN_ARR_ID"
	...
	def inserta_fitxa(self, fitxa, valors):
		...
		cin = ",".join(ci)
		#~ vals = ",".join(va)
		sent = "INSERT INTO %s (%s) VALUES(%s)" % (self.taules[fitxa], cin,'?'+ ',?'*(len(ci)-1))
		#~ sent = "INSERT INTO %s (%s) VALUES(%s)" % (self.taules[fitxa], cin,vals)
		print "SQL ",sent, va
		
		self.SQLINS[fitxa].sqlmod(sent,va)
